...
class CharacterOrganizer:
        ...
        try:
            ...
            if has_mixed_layout:
                ...
                organized_chars = self._organize_mixed_layout_characters(
                    valid_chars, centers, boxes, vertical_chars)
            elif is_multiline:
                if self.save_debug_images:
    ...
    def _organize_mixed_layout_characters(self, characters, centers, boxes, vertical_chars):
        ...
        if len(vertical_indices) == 0:
            return self._organize_single_line_characters(characters, centers, vertical_chars)
        ...
        if len(horizontal_chars) > 0:
            h_sorted_indices = np.argsort(horizontal_centers[:, 0])
        ...
        v_sorted_indices = np.argsort(vertical_centers_array[:, 1])
    ...
    def _organize_multiline_simple_and_robust(self, characters, centers, reference_height):
        ...
        if len(y_sorted) < 2:
            ...
            return [characters[idx] for idx in x_sorted_indices]
        ...
        if largest_gap > reference_height * 0.4:
            ...
            # Sort each line by X coordinate (with Y as secondary key for stability)
